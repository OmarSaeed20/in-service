...
import 'package:in_service/core/widget/dialogs/permission_dialog.dart';
import 'package:in_service/features/select_map_location/data/repositories/select_map_location_repo_imp.dart';
import 'package:in_service/index.dart';

part 'map_state.dart';

abstract class SelectMapLocationController extends GetxController {
...
class SelectMapLocationControllerImp extends SelectMapLocationController {
  SelectMapLocationControllerImp(/* {required this.locationRepo} */);
  // final SelectMapLocationRepositoryImp locationRepo;
  final Rx<MapState> _state = MapState(
    controller: Completer<GoogleMapController>(),
    location: const LatLng(30.06263, 31.24967),
  ).obs;

  MapState get state => _state.value;
  /*  Position? _position;
  ...
  Future<AddressModel> getCurrentLocation(
    ...
    Future<ZoneResponseModel> getZone(
        String? lat, String? long, bool markerLoad,
    ...
    Future<ResponseModel> addAddress(AddressModel addressModel,
        bool fromCheckout, bool fromRide, int? storeZoneId) async {
      _isLoading = true;
      update();
    ...
    void saveData(AddressModel address, bool fromSignUp, String? route,
      ...
      if (!GetPlatform.isWeb) {
        if (getUserAddress() != null) {
          if (getUserAddress()!.zoneIds != null) {
            for (int zoneID in getUserAddress()!.zoneIds!) {
...
*/
  void checkPermission(Function onTap) async {
    ...
    if (permission == LocationPermission.denied) {
      ToastManager().showSnak('you have to allow'.tr,isSuc: false);
